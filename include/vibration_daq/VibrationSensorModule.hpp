...
/**
 * The VibrationSensorModule handles the whole communication of one sensor over
 * SPI.
 */
class VibrationSensorModule {
 private:
  ...
  RecordingMode currentRecordingMode =
      RecordingMode::MTC;  // default for sensor as well
  ...
  bool parseRtsData(std::vector<uint16_t> &resp,
                    VibrationData &vibrationData) const;
  VibrationData readRtsSamplesBuffer(
      int samplesCount,
      const std::function<float(int16_t)> &convertVibrationValue) const;
  std::vector<float> readSamplesBuffer(
  ...
  bool writeRecordingControl(const RecordingMode &recordingMode,
  ...
  bool activateMode(
      ...
      const WindowSetting &windowSetting = WindowSetting::HANNING);
 ...
 public:
   ...
   * @param speed in Hz
   * @return true == successful setup, right model (ADcmXL3021) is connected and
   * the connection works
   */
  [[nodiscard]] bool setup(unsigned int resetPin, unsigned int busyPin,

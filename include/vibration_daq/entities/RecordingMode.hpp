...
enum class RecordingMode { MFFT = 0u, AFFT = 1u, MTC = 2u, RTS = 3u };
...
inline static const bool convert(const RecordingMode &fromEnum,
                                 std::string &toEnumString) {
  return convert(fromEnum, toEnumString, RECORDING_MODE_STRING_MAP);
}

inline static const bool convert(const std::string &fromEnumString,
                                 RecordingMode &toEnum) {

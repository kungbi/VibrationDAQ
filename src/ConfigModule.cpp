...
bool ConfigModule::readVibrationSensor(const YAML::Node &node,
  ...
  if (!Enum::convert(recordingModeString, vibrationSensor.recordingMode)) {
    LOG_S(WARNING) << "could not convert decimation_factor to enum: "
                   << recordingModeString;
    return false;
  }
...
bool ConfigModule::readRecordingConfig(const YAML::Node &node,
  ...
  if (!Enum::convert(decimationFactorString,
                     recordingConfig.decimationFactor)) {
    LOG_S(WARNING) << "could not convert decimation_factor to enum: "
                   << decimationFactorString;
    return false;
  }
  ...
  if (!Enum::convert(firFilterString, recordingConfig.firFilter)) {
    LOG_S(WARNING) << "could not convert fir_filter to enum: "
                   << firFilterString;
    return false;
  }

  if (recordingConfig.firFilter == FIRFilter::CUSTOM) {
    if (!convertNode(node["custom_filter_taps"],
                     recordingConfig.customFilterTaps)) {
...
bool ConfigModule::readMFFTConfig(const YAML::Node &node,
  ...
  if (mfftConfig.spectralAvgCount < 1 || mfftConfig.spectralAvgCount > 255) {
    LOG_S(WARNING) << "spectral_avg_count is not in range (1-255): "
                   << mfftConfig.spectralAvgCount;
    return false;
  }
  ...
  if (!Enum::convert(windowSettingString, mfftConfig.windowSetting)) {
    LOG_S(WARNING) << "could not convert window_setting to enum: "
                   << windowSettingString;
    return false;
  }
...
bool ConfigModule::readVibrationSensors(
    std::vector<VibrationSensorConfig> &vibrationSensors) {
...
bool ConfigModule::readStorageDirectoryPath(
    std::string &storageDirectory) const {
  return convertNode(configNode["storage_directory"], storageDirectory);
}

bool ConfigModule::readExternalTriggerConfig(bool &externalTriggerActivated,
                                             int &externalTriggerPin) const {
...
bool ConfigModule::readStatusLedConfig(bool &statusLedActivated,
                                       int &statusLedPin) const {

...
int main(int argc, char *argv[]) {
  ...
  if (!configModule.readExternalTriggerConfig(externalTriggerActivated,
                                              externalTriggerPin)) {
  ...
  if (externalTriggerActivated) {
    ...
    if (gpio_open(gpioTrigger, "/dev/gpiochip0", externalTriggerPin,
                  GPIO_DIR_OUT_LOW) < 0) {
  ...
  if (statusLedActivated) {
    ...
    if (gpio_open(gpioStatusLed, "/dev/gpiochip0", statusLedPin,
                  GPIO_DIR_OUT_LOW) < 0) {
  ...
  for (int i = 0; i < recordingsCount || recordingsCount == 0; ++i) {
    system_clock::time_point triggerTime =
        triggerVibrationSensors(externalTriggerActivated);
    ...
    for (const auto &vibrationSensorModule : vibrationSensorModules) {
      ...
      bool storedVibrationData = storageModule.storeVibrationData(
          vibrationData, vibrationSensorModule.getSensorName(), triggerTime);
...
system_clock::time_point triggerVibrationSensors(const bool &externalTrigger) {
  ...
  if (externalTrigger) {
    ...
    for (const auto &vibrationSensorModule : vibrationSensorModules) {
      // start recording
      LOG_S(INFO) << vibrationSensorModule.getSensorName()
                  << " triggered over SPI.";
...
bool setupVibrationSensorModules(const bool &externalTriggerActivated) {
  ...
  for (const auto &vibrationSensorConfig : vibrationSensorConfigs) {
    ...
    if (!vibrationSensorModule.setup(
            vibrationSensorConfig.resetPin, vibrationSensorConfig.busyPin,
            vibrationSensorConfig.spiPath, SPI_SPEED)) {
      LOG_S(ERROR) << "Could not setup vibration sensor: "
                   << vibrationSensorConfig.name;
      return false;
    }
